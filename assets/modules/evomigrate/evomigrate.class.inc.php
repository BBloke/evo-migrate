		...
		$output = "<table class='myTable'>";
			...
			$output .= "<tbody>";
				$output .= "<tr>";
					...
					$output .= "<td><input id='checkUsers' type='submit' name='checkUsers' value='Check Users' /><span></span></td>";
				$output .= "</tr>";
				...
				$output .= "<tr>";
					...
					$output .= "<td><input id='installTables' type='submit' name='installTables' value='Install Tables' /><span></span></td>";
				$output .= "</tr>";
				
				$output .= "<tr>";
					$output .= "<td>Setup New System Events</td>";
					$output .= "<td><input id='systemEvents' type='submit' name='systemEvents' value='Setup System Events' /><span></span></td>";
				$output .= "</tr>";
				
				$output .= "<tr>";
					$output .= "<td>Migrate Users</td>";
					$output .= "<td><input id='migrateUsers' type='submit' name='migrateUsers' value='Migrate Users' /><span></span></td>";
				$output .= "</tr>";
				
				$output .= "<tr>";
					$output .= "<td>Download and install v3</td>";
					$output .= "<td><input id='installv3' type='submit' name='installv3' value='Install v3' /><span></span></td>";
				$output .= "</tr>";
		...
		$output .= '<script type="text/javascript">
						...
						$("#checkUsers").click(function () {
							$.ajax({
								...
								success		: function( html ) {
									$("#checkUsers").closest("td").find("span").empty().append(html);
						...
						$("#installTables").click(function () {
							$.ajax({
								...
								success		: function( html ) {
									$("#installTables").closest("td").find("span").empty().append(html);
						...
						$("#systemEvents").click(function () {
							$.ajax({
								...
								success		: function( html ) {
									$("#systemEvents").closest("td").find("span").empty().append(html);
						...
						$("#migrateUsers").click(function () {
							$.ajax({
								...
								success		: function( html ) {
									$("#migrateUsers").closest("td").find("span").empty().append(html);
						...
						$("#installv3").click(function () {
							$.ajax({
								...
								success		: function( html ) {
									$("#installv3").closest("td").find("span").empty().append(html);
						...
						function returnMsg ( strMsg ) {
							$(".myTable td:next").append(strMsg);
						}
					</script>';
...
if ( empty($_REQUEST['action']) ) {
	...
	$output .=  '<input type="hidden" name="a" value="'.$action_id.'"/>';
    $output .=  '<input type="hidden" name="id" value="'.$module_id.'"/>';
	$output .=  "<input type='submit' name='action' value='Run' class='btn'>";
	$output .=  "</form>";
	...
	if ( $nameCount > 0 && $userCount > 0 ) {
		...
		while ( $row = $modx->db->getRow($rsWebGroups) ) {
			...
			$rsGroupDuplicate = $modx->db->select( "name", $modx->getFullTableName('membergroup_names'), "name='".$row['name']."'", "id ASC");
			if ( $modx->db->getRecordCount($rsGroupDuplicate) > 0 ) {
				$output .=  " <srtong>duplicate found!</strong><br />";
			} else {
				$modx->db->insert(array( "name" => $row['name']), $modx->getFullTableName('membergroup_names') );
			}
			$output .=  "<br />";
...
$output .=  "Migrating Managers to Users<br />";
...
(36,	'2018_06_29_182342_create_webgroup_names_table',	1);";
	...
	$output .=  "Created migrations_install!<br />";
}
// END MIGRATION INSTALL //
...
if ( $count == 0 ) {
  ...
  PRIMARY KEY (`id`)
	...
	$output .=  "<hr />Created permissions!<br />";
}
// END PERMISSIONS //
...
;";
	...
	$output .=  "Created role_permissions!<br />";
}
// END ROLE PERMISSIONS //
